...
export class BreadthFirstKinshipCalculator implements KinshipCalculator {
  ...
  private findAllShortestPreferredPaths(
    ...
    const pathsWithInlawCount = new Map<string, { path: string[]; nodePath: string[]; inlawCount: number }>();
    pathsWithInlawCount.set(centerId, { path: [], nodePath: [centerId], inlawCount: 0 });
    ...
    while (head < queue.length) {
      ...
      const currentPathObj = pathsWithInlawCount.get(currentId)!;
      ...
      for (const neighbor of neighbors) {
        ...
        const neighborIsInlaw = neighbor.direction === 'inlaw' ? 1 : 0;

        if (prevDepth === undefined || nextDepth < prevDepth) {
          visitedDepth.set(neighbor.id, nextDepth);
          pathsWithInlawCount.set(neighbor.id, {
            path: [...currentPathObj.path, neighbor.direction],
            nodePath: [...currentPathObj.nodePath, neighbor.id],
            inlawCount: currentPathObj.inlawCount + neighborIsInlaw,
          });
          queue.push(neighbor.id);
        } else if (nextDepth === prevDepth) {
          const currentInlawCount = currentPathObj.inlawCount + neighborIsInlaw;
          const existingPathObj = pathsWithInlawCount.get(neighbor.id);
    ...
    const paths = new Map<string, { path: string[]; nodePath: string[] }>();
    for (const [id, pathObj] of pathsWithInlawCount.entries()) {
