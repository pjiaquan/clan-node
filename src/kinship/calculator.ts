...
type TraversalStep = {
  ...
  path: string[];
  nodePath: string[];
  // Bolt Optimization: Track inlawCount incrementally (O(1)) instead of calling O(N) .filter().length inside traversal loops
  inlawCount?: number;
};
...
export class BreadthFirstKinshipCalculator implements KinshipCalculator {
    ...
    while (head < queue.length) {
      ...
      if (current.id === targetId) {
        if (foundDepth === null) {
          ...
          bestPath = { path: current.path, nodePath: current.nodePath, inlawCount: current.inlawCount || 0 };
        } else if (current.path.length === foundDepth) {
          const currentInlawCount = current.inlawCount || 0;
      ...
      for (const neighbor of neighbors) {
        ...
        if (!visited.has(neighbor.id) || canRevisitTarget) {
          ...
          queue.push({
            ...
            inlawCount: (current.inlawCount || 0) + (neighbor.direction === 'inlaw' ? 1 : 0),
          });
        }
      }
    }
  ...
  private findAllShortestPreferredPaths(
    ...
    const paths = new Map<string, { path: string[]; nodePath: string[]; inlawCount: number }>();
    paths.set(centerId, { path: [], nodePath: [centerId], inlawCount: 0 });
      ...
      for (const neighbor of neighbors) {
        ...
        if (prevDepth === undefined || nextDepth < prevDepth) {
          ...
          paths.set(neighbor.id, {
            ...
            inlawCount: (currentPathObj.inlawCount || 0) + (neighbor.direction === 'inlaw' ? 1 : 0),
          });
          queue.push(neighbor.id);
        } else if (nextDepth === prevDepth) {
          const currentInlawCount = (currentPathObj.inlawCount || 0) + (neighbor.direction === 'inlaw' ? 1 : 0);
          const existingPathObj = paths.get(neighbor.id);
          const existingInlawCount = existingPathObj
            ? (existingPathObj.inlawCount || 0)
            : Number.POSITIVE_INFINITY;
    ...
    // Explicitly strip inlawCount for structural subtyping safety on return type Map<K, V>
    const finalPaths = new Map<string, { path: string[]; nodePath: string[] }>();
    for (const [id, value] of paths.entries()) {
      finalPaths.set(id, { path: value.path, nodePath: value.nodePath });
    }
    return finalPaths;
  }
}
