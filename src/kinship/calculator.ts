...
export class BreadthFirstKinshipCalculator implements KinshipCalculator {
      ...
      for (const parentId of parents) {
        if (!visited.has(parentId)) {
          ...
          queue.push({
            ...
            inlawCount: current.inlawCount, // 'up' is never an inlaw
          });
        }
      }
    }
  ...
  private findShortestPreferredPath(centerId: string, targetId: string, adjacency: Map<string, AdjacencyEdge[]>) {
    // ⚡ Bolt Optimization: Initialize and track `inlawCount` incrementally.
    // Replaces O(N) `.filter().length` with O(1) integer tracking in tie-breakers.
    const queue: TraversalStep[] = [{ id: centerId, path: [], nodePath: [centerId], inlawCount: 0 }];
    const visited = new Set<string>([centerId]);
    ...
    while (head < queue.length) {
      ...
      if (current.id === targetId) {
        if (foundDepth === null) {
          ...
          // ⚡ O(1) tracking eliminates array iteration in tight BFS loops
    ...
    if (bestPath) {
      return { path: bestPath.path, nodePath: bestPath.nodePath };
    }
    return null;
  }
  ...
  private findAllShortestPreferredPaths(
    ...
    // ⚡ Bolt Optimization: Uses `internalPaths` to cache O(1) inlaw increments.
    // Significantly reduces latency for batch tie-breakers in large overlapping subgraphs.
