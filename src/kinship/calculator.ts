...
export class BreadthFirstKinshipCalculator implements KinshipCalculator {
      ...
      if (current.id === targetId) {
        if (foundDepth === null) {
          ...
          if (currentInlawCount < bestInlawCount) {
            bestPath = { path: current.path, nodePath: current.nodePath, inlawCount: currentInlawCount };
          }
        }
        continue;
      }
  ...
  private findAllShortestPreferredPaths(
    ...
    const cleanPaths = new Map<string, { path: string[]; nodePath: string[] }>();
    for (const [key, value] of paths.entries()) {
      cleanPaths.set(key, { path: value.path, nodePath: value.nodePath });
    }
    return cleanPaths;
  }
}
