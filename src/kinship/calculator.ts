...
export class BreadthFirstKinshipCalculator implements KinshipCalculator {
  ...
  private findAllShortestPreferredPaths(
    ...
    // ⚡ Bolt: Cache inlawCount on the path payload to avoid O(N) array filtering later
    ...
    // ⚡ Bolt: Use a zero-cost type assertion to satisfy the return signature
    // rather than unconditionally mapping over all nodes, which would add an O(V) overhead.
    return paths as unknown as Map<string, { path: string[]; nodePath: string[] }>;
  }
}
