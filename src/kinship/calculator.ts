...
export class BreadthFirstKinshipCalculator implements KinshipCalculator {
  ...
  private findShortestPreferredPath(centerId: string, targetId: string, adjacency: Map<string, AdjacencyEdge[]>) {
    ...
    // ⚡ Bolt: Track inlawCount in bestPath to avoid O(N) recalculations
    ...
    while (head < queue.length) {
      ...
      if (current.id === targetId) {
        if (foundDepth === null) {
          ...
          // ⚡ Bolt: O(1) comparison using incremental counter
          if (current.inlawCount < bestPath!.inlawCount) {
            bestPath = { path: current.path, nodePath: current.nodePath, inlawCount: current.inlawCount };
          }
        }
        continue;
      ...
      for (const neighbor of neighbors) {
        ...
        if (!visited.has(neighbor.id) || canRevisitTarget) {
          ...
          // ⚡ Bolt: Track inlawCount incrementally O(1)
          queue.push({
            id: neighbor.id,
    ...
    // Strip inlawCount to match the return type implicitly
    return bestPath ? { path: bestPath.path, nodePath: bestPath.nodePath } : null;
  }
  ...
  private findAllShortestPreferredPaths(
    ...
    // ⚡ Bolt: Use an internal Map tracking inlawCount to prevent O(N) array filtering
    ...
    while (head < queue.length) {
      ...
      for (const neighbor of neighbors) {
        ...
        if (prevDepth === undefined || nextDepth < prevDepth) {
          ...
          const existingInlawCount = existingPathObj ? existingPathObj.inlawCount : Number.POSITIVE_INFINITY;
    ...
    // ⚡ Bolt: Map to expected return type without extra properties to satisfy TS Map invariance
    const paths = new Map<string, { path: string[]; nodePath: string[] }>();
    for (const [id, pathObj] of internalPaths) {
