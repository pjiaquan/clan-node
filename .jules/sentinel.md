...
## 2025-05-18 - [DoS via Missing Maximum Length Checks on Optional Password Fields]
**Vulnerability:** Attackers could bypass DoS mitigations in password hashing (PBKDF2) by sending excessively long strings to authentication endpoints (e.g., login, reset-password, and admin updates) that were missing maximum length validations.
**Learning:** Security validations (like maximum length checks on passwords) were applied inconsistently across different handlers. Additionally, when backfilling these validations to endpoints where the field is optional (like admin updates), directly accessing `.length` on `undefined` causes functional regressions (`TypeError`).
**Prevention:** Apply defensive maximum length bounds consistently across all ingress points that lead to computationally expensive algorithms. Always use truthiness checks (e.g., `if (password && password.length > MAX)`) before validating properties on optional fields.
